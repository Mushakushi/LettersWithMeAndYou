Shader "Mushakushi/Nature/SpeedTree8"
{
    Properties
    {
        ...
        _Glossiness ("Smoothness", Range(0.0, 1.0)) = 0.0 // (shouldn't be very smooth at all)
        ...
        [Enum(No,2,Yes,0)] _TwoSided ("Two Sided", Int) = 0 // enum matches cull mode (if not two sided the tree will not render correctly, obviously)
        [KeywordEnum(None,Fastest,Fast,Better,Best,Palm)] _WindQuality ("Wind Quality", Range(0,5)) = 0
        
        // here only to provide default values 
        [HideInInspector] _TranslucencyColor ("Trans. Color", Color) = (1,1,1,1)
        [HideInInspector] _TranslucencyViewDependency ("Trans. View Dep.", Float) = 0 
        [HideInInspector] _Cutoff ("Cutoff", Float) = 0
        [HideInInspector] _ShadowStrength ("Shadow Str.", Float) = 0
    }
